...
The server also supports **standard OAuth2** for better MCP client compatibility, eliminating the need for the manual `login` tool.

### OAuth2 Flow

1. **Client Configuration** → Configure OAuth2 client with server endpoints
2. **Authorization Request** → Client redirects user to authorization endpoint
3. **User Authentication** → User logs in via web UI with email/password
4. **Authorization Code** → Server returns auth code to client callback
5. **Token Exchange** → Client exchanges code for access/refresh tokens
6. **API Access** → Client sends `Authorization: Bearer <token>` header
7. **Pre-Authentication** → Server automatically validates token and pre-authenticates session
...
Returns OAuth2 server metadata for automatic client configuration.
...
Initiates OAuth2 flow. Redirects to login UI, then to callback with auth code.
...
### MCP Client OAuth2 Configuration
  ...
  "mcpServers": {
    "iot-cloud": {
      ...
      "oauth2": {
        ...
        "authorization_url": "http://localhost:3001/oauth/authorize",
...
### OAuth2 vs Login Tool

| Method         | Pros                                                                                                      | Cons                                                 |
| -------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **OAuth2**     | ✅ Standard protocol<br>✅ Better client support<br>✅ Automatic token refresh<br>✅ No manual login tool | ❌ More complex setup                                |
| **Login Tool** | ✅ Simple to use<br>✅ Works with any MCP client                                                          | ❌ Manual authentication required<br>❌ Not standard |

**Recommendation**: Use OAuth2 for production deployments and login tool for development/testing.

### Environment Variables for OAuth2

```bash
# OAuth2 Configuration
BASE_URL=http://localhost:3001          # Base URL for OAuth endpoints
AUTH_REQUEST_TTL=600                   # Auth request TTL (seconds)
AUTH_CODE_TTL=60                       # Auth code TTL (seconds)
```
